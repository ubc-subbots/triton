...
public:
     ...
     */
    vector<vector<float>> featurize_for_classification(vector<vector<Point>> hulls)
    {
        vector<vector<float>> X_hat;
        for (vector<Point> hull : hulls)
        {
            X_hat.push_back(form_feature_vector(hull));
        }
        return X_hat;
    }
